namespace MVC.Framework.Routers
{
    ...
    public class ControllerRouter : IHandleable
    {
        ...
        {
            foreach (var method in GetSuitableMethods())
            {
        ...
        private void PrepareMethodParameters(MethodInfo methodInfo)
        {
            ...
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                {
                    var getParamaterValue = this.getParameters[parameter.Name];

                    var value = Convert.ChangeType(getParamaterValue, parameter.ParameterType);
