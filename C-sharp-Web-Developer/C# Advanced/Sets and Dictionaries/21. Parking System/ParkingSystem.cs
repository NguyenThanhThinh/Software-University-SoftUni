...
* 
* In order to give 100/100 in Judge we need to use Hashset<string> or List<Hashset<int>>. In the first case we need parse to string the needed coordinates
* and added them as a string. In the second scenario, we add in every Hashset<int> (Which in the List is equal to the searched Row) the specific Col
* which we are searching now and check it, if there is, that means the place is busy. I do not re-wrote the code, because I understanding well and I don't
* think that I will encountered "underwater stones"
* */
